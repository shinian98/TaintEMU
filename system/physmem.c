...
static void ram_block_add(RAMBlock *new_block, Error **errp)
{
    ...
    if (!new_block->host) {
        if (xen_enabled()) {
            ...
            new_block->host_shadow = qemu_anon_ram_alloc(new_block->max_length,
                                                  &new_block->mr->align,
                                                  shared, noreserve);
            if (!new_block->host_shadow) {
                error_setg_errno(errp, errno,
                                 "cannot set up guest shadow memory '%s'",
                                 memory_region_name(new_block->mr));
                return;
            }
            new_block->shadow_addend = new_block->host_shadow - new_block->host;
            #endif
            if (!new_block->host) {
                error_setg_errno(errp, errno,
