                       ...
                       vaddr addr, CPUTLBEntryFull *full)
{
    ...
    if (is_ram || is_romd) {
        ...
        #ifdef CONFIG_TCG_TAINT
        shadow_addend = (intptr_t) memory_region_get_shadow_addend (section->mr);
        #endif
    } else {
    ...
    #ifdef CONFIG_TCG_TAINT
    tn.shadow_addend = shadow_addend;
    #endif
...
typedef struct MMULookupPageData {
    ...
    void *haddr;
    #ifdef CONFIG_TCG_TAINT
    void *saddr; // shadow memory address
    #endif
    vaddr addr;
    int flags;
    int size;
                        ...
                        int mmu_idx, MMUAccessType access_type, uintptr_t ra)
{
    ...
    #ifdef CONFIG_TCG_TAINT
    data->saddr = data->haddr + entry->shadow_addend;
    #endif
                          ...
                          uintptr_t ra, MMUAccessType access_type)
{
    ...
    #ifdef CONFIG_TCG_TAINT
    cpu->qemu_ld_temp = do_ld_s_1 (cpu, &l.page[0], l.mmu_idx, access_type, ra);
    #endif
                           ...
                           uintptr_t ra, MMUAccessType access_type)
{
    ...
    if (likely(!crosspage)) {
        #ifdef CONFIG_TCG_TAINT
        cpu->qemu_ld_temp = do_ld_s_2 (cpu, &l.page[0], l.mmu_idx, access_type, l.memop, ra);
        #endif
    ...
    #ifdef CONFIG_TCG_TAINT
    sa = do_ld_s_1(cpu, &l.page[0], l.mmu_idx, access_type, ra);
    sb = do_ld_s_1(cpu, &l.page[1], l.mmu_idx, access_type, ra);
                           ...
                           uintptr_t ra, MMUAccessType access_type)
{
    ...
    if (likely(!crosspage)) {
        #ifdef CONFIG_TCG_TAINT
        cpu->qemu_ld_temp = do_ld_s_4 (cpu, &l.page[0], l.mmu_idx, access_type, l.memop, ra);
        #endif
    ...
    #ifdef CONFIG_TCG_TAINT
    sret = do_ld_s_beN(cpu, &l.page[0], 0, l.mmu_idx, access_type, l.memop, ra);
    sret = do_ld_s_beN(cpu, &l.page[1], sret, l.mmu_idx, access_type, l.memop, ra);
    if ((l.memop & MO_BSWAP) == MO_LE) {
        sret = bswap32(sret);
    }
    cpu->qemu_ld_temp = sret;
                           ...
                           uintptr_t ra, MMUAccessType access_type)
{
    ...
    if (likely(!crosspage)) {
        #ifdef CONFIG_TCG_TAINT
        cpu->qemu_ld_temp = do_ld_s_8(cpu, &l.page[0], l.mmu_idx, access_type, l.memop, ra);
        #endif
...
/* !! Taint Tracking doesn't support 128 bit ld/st */
                       ...
                       MemOpIdx oi, uintptr_t ra)
{
    ...
    #ifdef CONFIG_TCG_TAINT
    do_st_s_1(cpu, &l.page[0], (uint8_t) cpu->qemu_st_temp, l.mmu_idx, ra);
    #endif
}
                       ...
                       MemOpIdx oi, uintptr_t ra)
{
    ...
    if (likely(!crosspage)) {
        ...
        #ifdef CONFIG_TCG_TAINT
        do_st_s_2(cpu, &l.page[0], (uint16_t) cpu->qemu_st_temp, l.mmu_idx, l.memop, ra);
        #endif
        return;
    }
    ...
    #ifdef CONFIG_TCG_TAINT
    uint8_t sa, sb;
    uint16_t sval = (uint16_t) cpu->qemu_st_temp;
    if ((l.memop & MO_BSWAP) == MO_LE) {
        sa = sval, sb = sval >> 8;
    } else {
        sb = sval, sa = sval >> 8;
    }
    do_st_s_1(cpu, &l.page[0], sa, l.mmu_idx, ra);
    do_st_s_1(cpu, &l.page[1], sb, l.mmu_idx, ra);
    #endif
}
                       ...
                       MemOpIdx oi, uintptr_t ra)
{
    ...
    if (likely(!crosspage)) {
        ...
        #ifdef CONFIG_TCG_TAINT
        do_st_s_4(cpu, &l.page[0], (uint32_t) cpu->qemu_st_temp, l.mmu_idx, l.memop, ra);
        #endif
        return;
    }
    ...
    #ifdef CONFIG_TCG_TAINT
    uint32_t sval = (uint32_t) cpu->qemu_st_temp;
    if ((l.memop & MO_BSWAP) != MO_LE) {
        sval = bswap32(sval);
    }
    sval = do_st_s_leN(cpu, &l.page[0], sval, l.mmu_idx, l.memop, ra);
    (void) do_st_s_leN(cpu, &l.page[1], sval, l.mmu_idx, l.memop, ra);
    #endif
}
                       ...
                       MemOpIdx oi, uintptr_t ra)
{
    ...
    if (likely(!crosspage)) {
        #ifdef CONFIG_TCG_TAINT
        do_st_s_8(cpu, &l.page[0], (uint64_t) cpu->qemu_st_temp, l.mmu_idx, l.memop, ra);
        #endif
    ...
    (void) do_st_leN(cpu, &l.page[1], val, l.mmu_idx, l.memop, ra);
    #ifdef CONFIG_TCG_TAINT
    uint64_t sval = cpu->qemu_st_temp;
    if ((l.memop & MO_BSWAP) != MO_LE) {
        sval = bswap64(sval);
    }
    sval = do_st_s_leN(cpu, &l.page[0], sval, l.mmu_idx, l.memop, ra);
